...
export interface StaggeredMenuSectionItem {
  label: string
  link: string
}
...
export interface StaggeredMenuSection {
  title: string
  items: StaggeredMenuSectionItem[]
}
...
export const StaggeredMenu: React.FC<StaggeredMenuProps> = ({
  position = 'right',
  colors = ['#f5f5f5', '#e5e7eb', '#d1d5db', '#9ca3af'],
  items = [],
  socialItems = [],
  sections = [],
  displaySocials = true,
  ...
  const getPersonalizedSections = useCallback(() => {
    ...
    personalizedSections.push({
      ...
      items: personalizedData.recentSubgroups.length > 0 
        ? personalizedData.recentSubgroups.map((s: any) => ({ label: s.label ?? s.name ?? s.slug ?? 'Subgroup', link: s.link ?? (s.slug ? `/subgroup/${s.slug}` : '#') }))
        : []
    })
    ...
    personalizedSections.push({
      ...
      items: personalizedData.likedPosts.length > 0 
        ? personalizedData.likedPosts.map((p: any) => ({ label: p.label ?? p.name ?? 'Post', link: p.link ?? '#' }))
        : []
    })
    ...
    personalizedSections.push({
      ...
      items: personalizedData.recentPosts.length > 0 
        ? personalizedData.recentPosts.map((p: any) => ({ label: p.label ?? p.name ?? 'Post', link: p.link ?? '#' }))
        : []
    })
  ...
  return (
    ...
    >
      <div ref={preLayersRef} className="sm-prelayers" aria-hidden="true">
        {(() => {
          const raw = colors && colors.length ? colors.slice(0, 4) : ['#efefef', '#d4d4d8']
          let arr = [...raw]
          if (arr.length >= 3) {
                ...
                <section className="sm-section" key={sec.title + i}>
                  ...
                  <ul className="sm-section-list" role="list">
                    {sec.items && sec.items.length ? (
                      sec.items.map((s, j) => (
                        <li key={s.label + j} className="sm-section-item">
                          <a href={s.link} className="sm-section-link" title={s.label}>
                            {s.label}
                          </a>
                        </li>
                      ))
